...
#DEFINE DROPPED_TABLE_STATUS_LOC "Table supprimée" && "Table Dropped"
...
* string, then call BulkXMLLoad.PRG.
	...
	function BulkXMLLoad(tcTableName, tcCursorName, tcRmtTableName)
		...
		if empty(This.DataSourceName)
			...
			loRegistry = newobject('ODBCReg', 'registry.vcx')
			loRegistry.EnumODBCData(@laValues, 'AccountMate Sample')
            ...
            *{ JEI RKR 24.11.2005 Add  
			do case
				case upper(alltrim(aTblFields[ii, 3])) $ 'DT' and aTblFields[ii, 4]
            		lcEmptyDateValue = "IIF(EMPTY(" + ALLT(lcCursorName)+'.'+ALLT(aTblFields[ii,1])+ "), .Null.,"
        			lcEmptyDateValueEnd = ")"    	
...
******************************            
			*!* Auteur: Michel LEVY
			*!* Date: 02/04/2008 15:38:06
			*!* Copyright: ATOUTFOX.ORG (libre de droit, domaine public)
			*!* Description: 	If a PK is involved more than once in FKs, check if already created, 
			*!* 				since an error is raised when attempting to create it again
            ...
            IF THIS.ExportDRI AND ;
                ...
                *has primary key, it's wrong: log error
                ...
                IF lcXPkey==lcNewPrimary OR EMPTY(lcXPkey)THEN
					...
					* and if PK not already created ! (Michel LEVY 02/04/2008 15:38:06)

                    LOCAL llPKexists as Boolean 
                    STORE .f. TO m.llPKexists
                    SELECT (lcEnum_Indexes)
                    LOCATE FOR UPPER(RTRIM(IndexName))==UPPER(lcParentLoc) ;
                        AND UPPER(RTRIM(RmtExpr))==UPPER(lcNewPrimary)
                    m.llPKexists =  &lcEnum_Indexes..Exported

					SELECT (lcEnumTables)
                    IF THIS.DoUpsize AND NOT m.llPKexists THEN
                        llRetVal=THIS.ExecuteTempSPT(lcSQL, @lnError, @lcErrMsg)
                        IF !llRetVal THEN
