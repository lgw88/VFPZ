...
********************************************************************************
Function ctlComRegister(m.pcFileName As String)
	...
	If m.lnHandle <> 0 Then
		...
		If apiGetProcAddress(m.lnHandle, "DllRegisterServer") <> 0
			...
			If _2CC11JLUG_DllRegisterServer() = S_OK Then
				m.llRetVal = TRUE
			Endif
...
Function ctlComUnregister(m.pcFileName As String)
	...
	If m.lnHandle <> 0 Then
		...
		If apiGetProcAddress(m.lnHandle, "DllRegisterServer") <> 0
			...
			If _2CC11JLUG_DllUnregisterServer() = S_OK Then
				m.llRetVal = TRUE
			Endif
...
Function ctlFGets(m.pnFileHandle As Integer, m.pnBytesToRead As Integer)
	...
	Local ;
		...
		m.lnCrLfPos As Integer, ;
		m.lnOffset As Integer
...
Function ctlFileTimeToDateTime(m.poFileTime As ctlFileTime)
	...
	Local ;
		m.loSystemTime As ctlSystemTime, ;
		m.lcSystemTime As String, ;
		m.lcFileTime As String, ;
		m.ltFileDateTime As Datetime
...
********************************************************************************
Function ctlFontHeightToPoints(m.pnHeight As Integer)

	Local m.lnDc As Integer, ;
...
********************************************************************************
Function ctlFontPointsToHeight(m.pnPoints As Integer)
...
Function ctlGetStringHeightFromHwnd(m.pcString, m.pnHwnd)
	...
	*!* Get font
	m.lnFont = apiSendMessage(m.pnHwnd, WM_GETFONT, 0, 0)
	*!* Get DC
...
Function ctlGuidToString32(m.pcGuid As String)
	...
	*!* Reorder GUID bytes
	m.lcGuidString = Substr(m.pcGuid, 4, 1) ;
		+ Substr(m.pcGuid, 3, 1) ;
		+ Substr(m.pcGuid, 2, 1) ;
		+ Left(m.pcGuid, 1) ;
		+ Substr(m.pcGuid, 6, 1) ;
...
********************************************************************************
Function ctlGuidToString36(m.pcGuid As String)
...
*!* This is faster that using the StringFromGuid2 API call
********************************************************************************
Function ctlGuidToString38(m.pcGuid As String)
...
*!* format "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
********************************************************************************
Function ctlGuidFromString(m.pcGuidString As String)
