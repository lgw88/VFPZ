...
*-*	GENERATE code to Set Field Level Properties
	lcDBCName = DBC()
	USE (lcDBCName) AGAIN IN 0 ALIAS gkkdbc SHARED
	SELECT gkkdbc
	LOCATE FOR ALLTRIM(UPPER(ObjectName)) == pcViewName .AND. ObjectType = 'View'
	lnObjectId = gkkdbc.ObjectId
	SELECT ObjectName FROM gkkdbc WHERE ParentId = lnObjectId INTO ARRAY lcViewFlds
	USE IN gkkdbc
	IF _TALLY = 0
		lnFldCnt = 0
