...
LOCAL lnBeg, lnEnd, lcToken, lcDelimiters
#DEFINE TAB  CHR(9)
lcDelimiters = "!@#$%&*()-+={}<,>.?/;" + SPACE(1) + TAB
lnBeg = 1
lnEnd = 1
...
*-*		Skip quoted strings
		...
		ENDIF
*-*		Process next token
		IF !(SUBSTR(tcCodeLine,lnBeg,1) $ lcDelimiters)
			lnEnd = lnBeg + 1
			DO WHILE lnEnd <= LEN(tcCodeLine)
				IF SUBSTR(tcCodeLine,lnEnd,1) $ lcDelimiters
					EXIT
				ENDIF
				lnEnd = lnEnd + 1
			ENDDO
