...
FUNCTION GKKGenOwnerDrawnTopLevel
LPARAMETERS toForm,tcFileName
LOCAL lcMenu, lnPad, lnPopUpID, lnNdx, lnNumPads, lnNumLines, lnLine, lnPopUpMask, lcLevelName
LOCAL ARRAY laPad[1], laCmd[1]
LOCAL ARRAY laPopupNames[1], laPadNames[1]
...
*-*	Define the menu initialization
	lcMenu =                 '*-*' + REPLICATE("*",95)
	lcMenu = lcMenu + CRLF + '*-*  Add this section to Init() method of form'
	lcMenu = lcMenu + CRLF + '*-*'
	lcMenu = lcMenu + CRLF + 'thisform.NewObject("ODMenu","PopupMenu","OwnerDrawn_Menu")'	
	lcMenu = lcMenu + CRLF + 'IF VARTYPE(thisform.ODMenu) != "O"'
	...
	lcMenu = lcMenu + CRLF + '*-*  Add this section for closing form on quit'
	lcMenu = lcMenu + CRLF + '*-*'
	lcMenu = lcMenu + CRLF + 'IF !ISNULL(thisform.ODMenu)'
	lcMenu = lcMenu + CRLF + '	UNBINDEVENTS(thisform.ODMenu.hWndParent)'
	lcMenu = lcMenu + CRLF + '	thisform.ODMenu = .NULL.'
	lcMenu = lcMenu + CRLF + 'ENDIF'
	lcMenu = lcMenu + CRLF + 'thisform.Closable = .T.'
	lcMenu = lcMenu + CRLF
	lcMenu = lcMenu + CRLF
	lcMenu = lcMenu + CRLF + '*-*' + REPLICATE("*",95)
	lcMenu = lcMenu + CRLF + '*-*  Add this to the defines file for the form'
	lcMenu = lcMenu + CRLF + '*-*'
...
*-*	Define the menu definition header
	lcMenu = lcMenu + CRLF + '*-*' + REPLICATE("*",95)
	lcMenu = lcMenu + CRLF + '*-*  Add this Procedure for defining the Owner Drawn menu system'
	lcMenu = lcMenu + CRLF + '*-*'
	lcMenu = lcMenu + CRLF + 'PROCEDURE DefineMenu'
	...
	lcMenu = lcMenu + CRLF + 'WITH thisform.ODMenu'
...
*	lcMenu = lcMenu + CRLF + '	.nTextHiliteColor = '   + TRANSFORM()
	...
	lcMenu = lcMenu + CRLF
*-*	Determine the number of levels in this menu
	SELECT MAX(OCCURS(".",wbslevel)) AS maxvalue FROM c_editmnu INTO CURSOR c_max
	lnPopUpMask = 10 ^ c_max.maxvalue
...
*-*	Add the menu pad popups
	...
	FOR lnPad=1 TO ALEN(laPad,1)
		...
		IF lnNumPads > 0
			...
			lnBar      = 0
			lnPopUpID  = lnPopUpMask * lnPopUpNdx + 1            && 100000
...
IF FOUND()
	lnPopUpID = c_subpopup.popupid + (c_subpopup.popupcnt * 10^(c_max.maxvalue+2-tnLevel))
ELSE
ENDIF
