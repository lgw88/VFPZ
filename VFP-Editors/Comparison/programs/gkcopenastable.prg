...
*-*					Prompt user for file path location
					...
					ENDIF
					DO FORM GKCLocFile WITH JUSTFNAME(tcClassLoc) TO lcClassLoc
					=INKEY(0.1,'H')
					IF VARTYPE(toForm) = "O" .AND. PEMSTATUS(toForm,"clsLightBox",5)
...
*-*						User selected location; add to alternate file paths
			...
			CASE INLIST(loException.ErrorNo,3,24,101,108) && File in use
				GKCDisplayMsg('Visual Class Library in use: "' + JUSTFNAME(lcClassLoc) + ';" cannot display object.')
				llExit = True

			CASE loException.ErrorNo = 15                 && Not a table
				GKCDisplayMsg('Visual Class Library files corrupted: "' + JUSTFNAME(lcClassLoc) + ';" cannot display object.')
				llExit = True

			CASE loException.ErrorNo = 41                 && Invalid memo file
				GKCDisplayMsg('Visual Class Library has invalid memo file: "' + JUSTFNAME(lcClassLoc) + ';" cannot display object.')
				llExit = True

			OTHERWISE
				GKCDisplayMsg('Unknown error occurred while trying to open "' + JUSTFNAME(lcClassLoc) + ';" cannot display object [ErrorNo: ' + TRANSFORM(loException.ErrorNo) + '].')
				llExit = True
		ENDCASE
	ENDTRY
ENDDO
