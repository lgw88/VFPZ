...
*-*	Get the stored procedures
	USE (pcDBCName) AGAIN IN 0 ALIAS gkpdbc SHARED
	SELECT gkpdbc
	LOCATE FOR Objectname = 'StoredProceduresSource'
	IF FOUND()
		lcSP = fStripLine(ALLTRIM(gkpdbc.code))
		lcDbcDefn = lcDbcDefn + "****StoredProceduresSource|" + lcSP + CRLF
		lcDbcDefn = lcDbcDefn + "USE '<<DBCName>>' AGAIN IN 0 ALIAS gkpdbc SHARED" + CRLF
		lcDbcDefn = lcDbcDefn + "SELECT gkpdbc" + CRLF
		lcDbcDefn = lcDbcDefn + "LOCATE FOR Objectname = 'StoredProceduresSource'" + CRLF
		lcDbcDefn = lcDbcDefn + "IF FOUND()" + CRLF
		lcDbcDefn = lcDbcDefn + "   REPLACE gkpdbc.code WITH lcSP" + CRLF
...
*-*	Get tables contained in database
	...
	IF lnNumObjs > 0
		...
		FOR lnNdx=1 TO lnNumObjs
			loReturn.Schema   = ""
			loReturn.VersCtrl = ""
			lcTblInfo = GKPGetTableDefn("",lcDbcObjs[lnNdx],loReturn,.F.)
...
*-*	Get views contained in database
	...
	IF lnNumObjs > 0
		...
		FOR lnNdx=1 TO lnNumObjs
			loReturn.Schema   = ""
			loReturn.VersCtrl = ""
			lcViewInfo = GKPGetViewDefn("",lcDbcObjs[lnNdx],loReturn,.F.)
			IF !EMPTY(loReturn.Schema) .AND. !EMPTY(loReturn.VersCtrl)
				lcDbcDefn  = lcDbcDefn  + loReturn.Schema + CRLF
...
*-*	Get connections contained in database
	...
	FOR lnNdx=1 TO lnNumObjs
		loReturn.Schema   = ""
		loReturn.VersCtrl = ""
		lcConnVers = GKPGetConnectionDefn("",lcDbcObjs[lnNdx],loReturn,.F.)
...
TRY
	...
	IF FOUND()
		...
		lcRIDefn = lcRIDefn + "USE '<<DBCName>>' IN 0 ALIAS gkpdbc EXCLUSIVE" + CRLF
		lcRIDefn = lcRIDefn + "SELECT gkpdbc" + CRLF
		DO WHILE FOUND()
			lnParentID = gkpdbc.ParentID
			SELECT ObjectName FROM (pcDBCName) WHERE ObjectID = lnParentID INTO ARRAY lcTblName
			lnStart = 1
			lcITag  = ""
			lcTable = ""
			lcRTag  = ""
			DO WHILE lnStart <= LEN(gkpdbc.property)
				lnSize = ASC(SUBSTR(gkpdbc.property,lnStart,1)) + (ASC(SUBSTR(gkpdbc.property,lnStart+1,1)) * 256) + ;
						(ASC(SUBSTR(gkpdbc.property,lnStart+2,1)) * 256^2) + (ASC(SUBSTR(gkpdbc.property,lnStart+3,1)) * 256^3)
				lnKey = ASC(SUBSTR(gkpdbc.property,lnStart+6,1))
				DO CASE
					CASE lnKey = 13
						lcITag = SUBSTR(gkpdbc.property,lnStart+7,lnSize-8)
					CASE lnKey = 18
						lcTable = SUBSTR(gkpdbc.property,lnStart+7,lnSize-8)
					CASE lnKey = 19
						lcRTag = SUBSTR(gkpdbc.property,lnStart+7,lnSize-8)
				ENDCASE
			...
			lcRIDefn = lcRIDefn + "    IF FOUND()" + CRLF
			lcRIDefn = lcRIDefn + "	      REPLACE RiInfo WITH '" + gkpdbc.RiInfo + "'" + CRLF
