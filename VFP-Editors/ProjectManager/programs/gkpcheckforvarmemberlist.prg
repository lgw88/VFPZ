...
*-*
FUNCTION GKPCheckForVarMemberList
LPARAMETERS poCodeMax,pnKeyAscii
...
*-*				Get the entered text by user
				...
				FOR lnCol=loRange.StartColNo TO 0 STEP -1
					...
					ENDIF
				ENDFOR
				lcVName = lcVName + CHR(pnKeyAscii)
				IF LEN(lcVName) > 2
*-*					Reset the member list
					...
					WITH loMbrList
						.Name = "VFPTempMbrList"
						.CaseSensitive = .F.
						.Language = poCodeMax.Language
*-*						Get the matching variables for current method
						lnMbrCnt = 0
						SELECT c_variables
						SCAN FOR ALLTRIM(UPPER(vmethod))==UPPER(poCodeMax.MethodName) .AND. UPPER(vname) = UPPER(lcVName)
							lnMbrCnt = lnMbrCnt + 1
